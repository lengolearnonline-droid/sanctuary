...
async function test() {
  const res = await fetch(SUPABASE_URL + '/rest/v1/sanctuary_activations?select=id,machine_id,license_key_id,activated_at,last_validated_at&limit=1', {
    method: 'GET',
    headers: {
  ...
  console.log('Activations:', await res.json());
}
test();
