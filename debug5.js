...
async function test() {
  const res = await fetch(SUPABASE_URL + '/rest/v1/sanctuary_subscriptions?select=full_name,email,plan,status,current_period_end&limit=1', {
    method: 'GET',
    headers: {
  ...
  console.log('Subscriptions:', await res.json());
}
test();
