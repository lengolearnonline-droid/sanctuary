...
async function test() {
  const res = await fetch(SUPABASE_URL + '/rest/v1/sanctuary_license_keys?select=id,max_activations,subscription_id,sanctuary_subscriptions(plan,status,current_period_end)&license_key=eq.SANC-AV9Y-QLLU-D5FF-AAXQ', {
    method: 'GET',
    headers: {
  ...
  console.log('Select result without is_active:', await res.json());
}
test();
